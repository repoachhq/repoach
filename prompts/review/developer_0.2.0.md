      ...
      "edits": [
        {
          ...
          "replace": "<the snippet as it should read after your change>"
        }
      ],
      "rationale": "<= 240 chars"
    }
...
- **Existing files: use ``edits``, never ``new_content``.** Each
  ...
  exactly once — include enough surrounding lines to make it unique.
  Edits apply in order. Re-emitting a whole existing file gets your
  fix rejected by the size guard.
- **New files: use ``new_content``** — the complete UTF-8 file
  contents.  No diff hunks, no partial files, no placeholder strings
  like ``"content1"`` or ``"<file contents here>"``.
