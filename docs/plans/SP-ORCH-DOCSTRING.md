...
```json
{
  ...
  "steps": [
    ...
    {
      "index": 2,
      "title": "Pin the docstring's truthfulness via the imported module (integration)",
      "files": [
        "tests/integration/test_orchestrator_docstring_integration.py"
      ],
      "action": "Create tests/integration/test_orchestrator_docstring_integration.py with a single test test_orchestrator_docstring_truthful_via_imported_module that imports ferova.review.orchestrator and asserts on the LIVE module __doc__ (not the file text): (a) none of the retired phrases appear ('Aggregates their verdicts', 'does **not** auto-merge', 'run_coder_response'); (b) the words 'findings', 'derived' and 'report-only' all appear. Module docstring on the test file explains it pins the SP-ORCH-DOCSTRING acceptance via the import path, complementing the unit-level source pins. Do not touch any other file.",
      "commit_message": "test(review): integration pin for the orchestrator docstring via import",
      "done_when": "pytest -q tests/integration/test_orchestrator_docstring_integration.py passes; ruff check tests/integration/test_orchestrator_docstring_integration.py exits 0.",
      "unit_tests": [
        "tests/unit/test_orchestrator_docstring.py::test_orchestrator_docstring_drops_the_retired_narrative",
        "tests/unit/test_orchestrator_docstring.py::test_orchestrator_docstring_describes_the_ledger_pipeline"
      ]
