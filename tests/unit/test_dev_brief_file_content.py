...
def test_helper_embeds_existing_file_content(tmp_path: Path) -> None:
    ...
    result = _embed_contract_files(["src/repoach/foo.py"], repo_root=tmp_path)

    assert "## Existing contract files" in result
    assert "### `src/repoach/foo.py`" in result
...
def test_helper_lists_missing_paths_under_to_create(tmp_path: Path) -> None:
    ...
    result = _embed_contract_files(
        ["src/repoach/foo.py", "tests/unit/test_foo_new.py", "docs/runbooks/foo.md"],
        repo_root=tmp_path,
    )
...
def test_helper_truncates_oversized_file_with_continuation_note(tmp_path: Path) -> None:
    ...
    from repoach.review.dev_runner import _EMBED_PER_FILE_CAP
    ...
    result = _embed_contract_files(["src/repoach/big.py"], repo_root=tmp_path)

    assert "### `src/repoach/big.py`" in result
    assert "[... truncated at" in result
    assert "read_file('src/repoach/big.py', start_line=" in result
...
def test_helper_handles_read_error_gracefully(tmp_path: Path) -> None:
    ...
    result = _embed_contract_files(["src/repoach/unreadable.py"], repo_root=tmp_path)

    assert "## Existing contract files" in result
    assert "### `src/repoach/unreadable.py`" in result
...
def test_helper_respects_total_budget(tmp_path: Path) -> None:
    ...
    from repoach.review.dev_runner import _EMBED_PER_FILE_CAP, _EMBED_TOTAL_BUDGET
    ...
    for i in range(file_count):
        name = f"f{i}.py"
        paths.append(f"src/repoach/{name}")
    ...
    assert "### `src/repoach/f0.py`" in result
...
def test_brief_embeds_existing_contract_files(tmp_path: Path, monkeypatch) -> None:
    ...
    plan, step = _one_step_plan(["src/repoach/foo.py"])
...
def test_oversized_file_truncated_with_continuation_note(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.chdir(tmp_path)
    plan, step = _one_step_plan(["src/repoach/big.py"])

    brief = build_step_brief(plan, step)

    assert "### `src/repoach/big.py`" in brief
    assert "[... truncated at" in brief
    assert "read_file('src/repoach/big.py', start_line=" in brief
