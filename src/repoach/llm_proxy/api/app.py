...
from repoach.llm_proxy.config.logging_config import configure_logging
...
def create_app() -> FastAPI:
    ...
    The factory wires the API router and three exception handlers:

    * :class:`RequestValidationError` — logs the request shape (no
      content values) so 422 errors stay debuggable;
    * :class:`ProviderError` — surfaces upstream provider failures
      in the Anthropic JSON shape;
    * any other :class:`Exception` — fall-through 500 in the same
      Anthropic envelope.
    """
    app = FastAPI(
...
app = create_app()
"""Default FastAPI app instance picked up by uvicorn."""
