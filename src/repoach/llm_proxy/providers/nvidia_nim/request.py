...
from repoach.llm_proxy.core.anthropic import build_base_request_body, set_if_not_none
from repoach.llm_proxy.providers.reasoning import bounded_reasoning_budget


def _set_extra(extra_body: dict[str, Any], key: str, value: Any, ignore_value: Any = None) -> None:
...
def clone_body_without_chat_template(body: dict[str, Any]) -> dict[str, Any] | None:
    """Clone a request body and strip the chat-template directives.

    NIM models backed by a Mistral tokenizer reject any chat-template
    directive with ``400 'chat_template is not supported for Mistral
    tokenizers.'``. The offending field is almost always
    ``chat_template_kwargs`` (the thinking block injected by
    :func:`build_request_body`), not the ``chat_template`` string. Strip
    BOTH so the one-shot 400 retry in the client genuinely downgrades the
    body; return ``None`` only when neither field was present (no point
    retrying). Mirrors :func:`clone_body_without_reasoning_budget`.
    """
    cloned_body = deepcopy(body)
    extra_body = cloned_body.get("extra_body")
    if not isinstance(extra_body, dict):
...
def build_request_body(request_data: Any, nim: NimSettings, *, thinking_enabled: bool) -> dict:
    """Build OpenAI-format request body from Anthropic request.

    The function then layers NIM-specific overrides on top of the
    base body:

    * ``max_tokens`` is capped against ``nim.max_tokens`` so a
      caller cannot request more than the operator allows.
    * ``temperature`` and ``top_p`` fall back to the NIM defaults
      when the request did not set them.
    * ``stop`` falls back to ``nim.stop`` when no stop sequences
      were supplied.
    * Non-standard parameters travel through ``extra_body`` so the
      OpenAI client can pass them through unchanged.
    """
    logger.debug(
    ...
    if thinking_enabled:
        ...
        bounded_budget = bounded_reasoning_budget(max_tokens)
        if isinstance(chat_template_kwargs, dict) and bounded_budget is not None:
            chat_template_kwargs.setdefault("reasoning_budget", bounded_budget)
    ...
    _set_extra(extra_body, "repetition_penalty", nim.repetition_penalty, ignore_value=1.0)
