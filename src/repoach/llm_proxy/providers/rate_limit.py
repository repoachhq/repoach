...
class GlobalRateLimiter:
    ...
    async def wait_if_blocked(self) -> bool:
        ...
        1. **Reactive** — wait if someone already hit a 429 and we
           are still inside the cool-down window.
        2. **Proactive** — enforce the strict rolling window (no
           bursts beyond ``rate_limit`` requests in the last
           ``rate_window`` seconds).

        Returns:
            ``True`` if the call waited reactively, ``False`` otherwise.
        """
    ...
    async def _acquire_proactive_slot(self) -> None:
        ...
        Guarantees at most ``self._rate_limit`` acquisitions in any
        interval of length ``self._rate_window`` (seconds).  When
        the window is full the call sleeps *outside* the lock so
        other tasks can continue to queue.
        """
        while True:
            wait_time = 0.0
        ...
        for attempt in range(1 + max_retries):
            ...
            try:
                ...
                if attempt >= max_retries:
                    logger.warning(f"Rate limit retry exhausted after {max_retries} retries")
                    break
