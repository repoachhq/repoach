...
def _serialize_value(value: Any) -> Any:
    ...
    if isinstance(value, dict):
        return {key: _serialize_value(item) for key, item in value.items() if item is not None}
...
def _apply_reasoning(body: dict[str, Any], thinking_cfg: Any) -> None:
    ...
    An explicit client ``thinking.budget_tokens`` wins; otherwise the
    reasoning budget defaults to the shared headroom bound
    (SP-CHAINPILOT-REASONING-WIRE-TOKEN), so a thinking request without a
    client budget still leaves room for the visible answer instead of
    letting reasoning consume the whole completion.
    """
    ...
    client_budget = thinking_cfg.get("budget_tokens") if isinstance(thinking_cfg, dict) else None
    if isinstance(client_budget, int):
        reasoning.setdefault("max_tokens", client_budget)
        return

    default_budget = bounded_reasoning_budget(body.get("max_tokens"))
    if default_budget is not None:
        reasoning.setdefault("max_tokens", default_budget)
...
def build_request_body(request_data: Any, *, thinking_enabled: bool) -> dict:
    ...
    body = {key: value for key, value in dumped_request.items() if key not in _INTERNAL_FIELDS}
