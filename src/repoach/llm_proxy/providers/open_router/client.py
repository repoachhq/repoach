...
from repoach.llm_proxy.core.anthropic import SSEBuilder, append_request_id
from repoach.llm_proxy.providers.anthropic_messages import (
    AnthropicMessagesTransport,
    StreamChunkMode,
)
from repoach.llm_proxy.providers.base import ProviderConfig
from repoach.llm_proxy.providers.defaults import OPENROUTER_BASE_URL
...
class OpenRouterProvider(AnthropicMessagesTransport):
    ...
    def _close_open_blocks_before(self, state: _SSEFilterState, upstream_index: int) -> str:
        ...
        for open_upstream_index in list(state.open_block_types):
            ...
            events.append(self._format_sse_event("content_block_stop", json.dumps(payload)))
    ...
    def _transform_sse_payload(
        ...
        *,
        ...
        if event_name == "content_block_start":
            ...
            upstream_index = payload.get("index")
            if self._should_drop_block_type(block_type, thinking_enabled=thinking_enabled):
        ...
        if event_name == "content_block_delta":
            ...
            if self._should_drop_block_type(delta_type, thinking_enabled=thinking_enabled):
                return None
        ...
        if event_name == "content_block_stop":
            upstream_index = payload.get("index")
            if isinstance(upstream_index, int) and upstream_index in state.closed_indexes:
    ...
    def _transform_stream_event(
        ...
        *,
        ...
        if isinstance(state, _SSEFilterState):
            event_name, data_text = self._parse_sse_event(event)
            if state.message_stopped or self._is_terminal_done_event(event_name, data_text):
                return None
        ...
        if thinking_enabled:
            if isinstance(state, _SSEFilterState):
                return self._transform_sse_payload(event, state, thinking_enabled=thinking_enabled)
            return event
        if isinstance(state, _SSEFilterState):
            return self._transform_sse_payload(event, state, thinking_enabled=thinking_enabled)
        return event
    ...
    def _emit_error_events(
        ...
        *,
        ...
        The terminal ``message_delta`` carries ``stop_reason="error"`` with
        ``output_tokens=0`` (not ``"end_turn"``/``1``) so the chain-failover
        peek layer (``api/_failover.py:peek_for_content``) classifies the
        emitted error text as a failure rather than as real model content.
        Without this, an upstream 4xx (e.g. a 402 "Insufficient credits")
        streamed as a ``text_delta`` would be served to the caller as the
