...
class OpenAIChatTransport(BaseProvider):
    ...
    async def _stream_response_impl(
        ...
        On any upstream exception, ``finish_reason`` is forced to
        ``"error"`` so :func:`map_stop_reason` propagates ``"error"``
        through the final ``message_delta``.  The chain-failover
        layer (``_failover.peek_for_content``) then classifies the
        stream as a failure and walks to the next chain candidate.
        Without this override ``finish_reason`` would stay ``None``,
        map to ``"end_turn"``, and let disguised error text leak
        downstream as if it were a real model answer (the HTTP 410
        EOL pattern observed on PR #125).
        """
