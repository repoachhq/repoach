...
class ThinkTagParser:
    ...
    def _parse_outside_think(self) -> ContentChunk | None:
        ...
        if think_start == -1:
            ...
            if last_bracket != -1:
                ...
                if (tag_len < len(self.OPEN_TAG) and self.OPEN_TAG.startswith(potential_tag)) or (
                    tag_len < len(self.CLOSE_TAG) and self.CLOSE_TAG.startswith(potential_tag)
                ):
    ...
    def _parse_inside_think(self) -> ContentChunk | None:
        ...
        if think_end == -1:
            last_bracket = self._buffer.rfind("<")
            if last_bracket != -1 and len(self._buffer) - last_bracket < len(self.CLOSE_TAG):
                potential_tag = self._buffer[last_bracket:]
    ...
    def flush(self) -> ContentChunk | None:
        ...
        if self._buffer:
            chunk_type = ContentType.THINKING if self._in_think_tag else ContentType.TEXT
