...
the model invents a ``repoach.*`` module, pytest answers
...
def _top_level_names(module_file: Path) -> set[str]:
    ...
    gate once refused ``from repoach.arch import load_registry`` —
    a re-export sanctioned by ``repoach.arch.__all__`` that pytest
...
def _find_name_homes(src_root: Path, name: str) -> list[str]:
    ...
    homes: list[str] = []
    for module_file in sorted((src_root / "repoach").rglob("*.py")):
        try:
            ...
            if isinstance(node, ast.Import):
                ...
                for alias in node.names:
                    ...
                    violations.append(
                        ...
                        f"found in: {', '.join(homes) or '(nowhere under repoach)'}"
                    )
    if violations:
        return False, "\n".join(violations)
    return True, ""
