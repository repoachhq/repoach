...
try:
    ...
    logger.warning("tiktoken_encoder_unavailable, token estimates degraded: {}", exc)
    ENCODER = None
...
def map_stop_reason(openai_reason: str | None) -> str:
    ...
    The ``"error"`` entry is preserved unchanged so the chain
    failover layer (``peek_for_content``) can detect upstream
    failures (HTTP 410 / 5xx / disguised connection errors) and
    walk to the next chain candidate, instead of forwarding a
    stream whose only "content" is the error text.
    """
...
class ContentBlockManager:
    ...
    def buffer_task_args(self, index: int, args: str) -> dict | None:
        ...
        try:
            ...
            logger.debug(
                "anthropic_sse.task_args_json_decode_failed",
                index=index,
...
class SSEBuilder:
    ...
    def emit_tool_delta(self, tool_index: int, partial_json: str) -> str:
        ...
        return self.content_block_delta(state.block_index, "input_json_delta", partial_json)
    ...
    def estimate_output_tokens(self) -> int:
        ...
        tool_tokens = sum(1 for state in self.blocks.tool_states.values() if state.started) * 50
