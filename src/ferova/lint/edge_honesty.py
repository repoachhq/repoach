...
from ferova.arch import Registry, has_frontmatter, load_registry
...
def load_frontier_suppress(repo_root: Path) -> frozenset[str]:
    ...
    ``[tool.ferova.arch] frontier_suppress`` lists artifacts (module
    ...
    arch = data.get("tool", {}).get("ferova", {}).get("arch", {})
...
def check_added_specs(added_specs: Iterable[str], repo_root: Path) -> list[SpecPresenceViolation]:
    ...
    concern — :func:`ferova.arch.load_registry` already raises on it.
